function calcularIMC() {
    ...
    let informacao;
    let pesoNormal;
    let peso1 = 18.5 * (alturaM * alturaM);
    let peso2 = 24.99 * (alturaM * alturaM);
    ...
    // Exibindo os resultados no elemento de resultado
    document.getElementById('resultado').innerHTML = 
        `<strong>Resultado:</strong><br>` +
        `<strong>IMC:</strong> ${imc.toFixed(2)}<br>` +
        `<strong>Classificação:</strong> ${classificacao}<br>` +
        `<strong>Peso normal:</strong> ${pesoNormal}<br>` +
        `<strong>Informação:</strong> ${informacao}`;
}
